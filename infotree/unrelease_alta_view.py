...
from infotree.models import unreleased_info_alta, info_tree_data_export_log, info_tree_data_import_temp, \
    unreleased_mdt, release_check
from infotree.models import tree, tree_option, info_tree_template, info_tree_template_option, cd_map, cec
from infotree.service.unreleased_alta_service import unreleased_alta_service
...
@csrf_exempt
def unreleased_alta_dict(request):
    tree_id_list = tree.objects.filter(is_delete=0).values()

    result_dict = {}
    for tree_id in tree_id_list:
        if tree_id['name'] == 'Tool':
            labels = tree_option.objects.filter(tree_id=tree_id['id'], is_delete=0, value__contains='MS03').order_by(
                'sort').values('value')
        else:
            labels = tree_option.objects.filter(tree_id=tree_id['id'], is_delete=0).order_by(
                'sort').values('value')
        label_list = []
        for label in labels:
...
@csrf_exempt
def unreleased_alta_edit_dict(request):
...
class unreleased_alta_list(BaseView):

    def get(self, request):
        if request.method == 'GET':
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_list.html')
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if customer and tool and node and tone and blank and grade and pattern_density and layer:
                ...
                info_tree_datalist = unreleased_info_alta.objects.filter(q).order_by('id').values()[
                                     self.startIndex: self.endIndex]

                data['total'] = unreleased_info_alta.objects.filter(q).count()
...
class unreleased_alta_view(BaseView):

    def get(self, request):
        id = request.GET.get("id")
        if not id:
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_form.html')
        else:
            _o = unreleased_info_alta.objects.get(id=id)
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_form.html',
                          {"unreleased_alta": _o})


class unreleased_alta_add(BaseView):
    def get(self, request):
        return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_add_form.html', {"method": "add"})
        ...
        if customer and tool and node and tone and blank and grade and pattern_density and layer:
            ...
            for _grade in grade_list:
                for _layer in layer_list:
                    data_count = unreleased_info_alta.objects.filter(tool=tool, node=node, tone=tone, blank=blank,
                                                                     grade__contains=_grade,
                                                                     pattern_density=pattern_density,
                                                                     layer__contains=_layer, is_delete=0,
            ...
            _s = unreleased_alta_service()

            return _s.add_unreleased_alta(_o)
...
class unreleased_alta_copy(BaseView):
    def get(self, request):
        ...
        return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_add_form.html',
                      {"method": "copy", "s_tool": s_tool, "s_node": s_node, "s_tone": s_tone, "s_blank": s_blank,
                       "s_grade": s_grade, "s_pattern_density": s_pattern_density, "s_layer": s_layer,
                       "s_level": s_level})
    ...
    def post(self, request):
        ...
        if s_customer and s_tool and s_node and s_tone and s_blank and s_grade and s_pattern_density and s_layer and \
                customer and tool and node and tone and blank and grade and pattern_density and layer and s_level and level:

            for _grade in grade_list:
                for _layer in layer_list:
                    data_count = unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone,
                                                                     blank=blank,
                                                                     grade__contains=_grade,
                                                                     pattern_density=pattern_density,
                                                                     layer__contains=_layer, is_delete=0,
            ...
            _o.id = [s_customer, s_tool, s_node, s_tone, s_blank, s_grade, s_pattern_density, s_layer, s_level]
            _o.customer = customer
            _o.tool = tool
            _o.node = node
            _o.tone = tone
            ...
            return _s.add_copy_unreleased_alta(_o)
        else:
            data = {"success": False, "msg": "Value must not be empty"}
            return JsonResponse(data, safe=False)


class unreleased_alta_edit(BaseView):
    def get(self, request):
        id = request.GET.get("id")
        if not id:
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_form.html', {"method": "edit"})
        else:
            _o = unreleased_info_alta.objects.get(id=id)
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_form.html',
                          {"method": "edit", "unreleased_alta": _o})
    ...
    def post(self, request):
        ...
        if sel_value or input_value:
            ...
            # elif _o.cs_command == 'CEC_PARAM':
            #     _o.cec_id = file_id

            _o.version = _o.version + 1
            _s = unreleased_alta_service()

            return _s.upd_unreleased_alta(_o)
        else:
            data = {"success": False, "msg": "Failed"}
            return JsonResponse(data, safe=False)


class unreleased_alta_del(BaseView):
    ...
    def get(self, request):
        ...
        return _s.del_unreleased_alta(_o)


def unreleased_alta_create_check(request):
    customer = request.POST.get('customer')
    tool = request.POST.get('tool')
    node = request.POST.get('node')
    tone = request.POST.get('tone')
    ...
    _o = unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone, blank=blank,
                                             grade=grade, pattern_density=pattern_density, layer=layer, level=level,
                                             is_delete=0).count()
...
@csrf_exempt
def unreleased_alta_check_value(request):
    cs_command = request.POST.get('item')
    value = request.POST.get('value')
    level = request.POST.get('level')
    id = request.POST.get('id')

    temp = info_tree_template.objects.get(cs_command=cs_command, level=level, is_delete=0)
        ...
        if case('dose'):
            ...
            if data[0]:
                if data[1] > 0:
                    ...
                    msg = 'dose out of range'
            else:
                msg = 'dose data type error'
            break
...
def unreleased_alta_tree_json(request):
    tree_type = int(request.GET.get('tree_type'))
    j_list = []
    customer_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0).values(
        'customer').distinct().order_by('customer')
    for customer in customer_list:
        # Tool
        tool_list = unreleased_info_alta.objects.filter(customer=customer['customer']).values(
        ...
        for tool in tool_list:
            ...
            # Node
            node_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0, tool=tool['tool'],
            ...
            for node in node_list:
                ...
                # Tone
                tone_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0, tool=tool['tool'],
                                                                node=node['node'],
                                                                customer=customer['customer']).values(
                    'tone').distinct()
                print(tone_list)
                ...
                for tone in tone_list:
                    ...
                    # Blank
                    blank_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0,
                                                                     tool=tool['tool'],
                                                                     node=node['node'],
                                                                     tone=tone['tone'],
                                                                     customer=customer['customer']).values(
                    ...
                    for blank in blank_list:
                        ...
                        # Grade
                        grade_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0,
                                                                         tool=tool['tool'],
                                                                         node=node['node'],
                                                                         tone=tone['tone'],
                                                                         blank=blank['blank'],
                        ...
                        for grade in grade_list:
                            ...
                            # Pattern density
                            pattern_density_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0,
                                                                                       tool=tool['tool'],
                                                                                       node=node['node'],
                                                                                       tone=tone['tone'],
                                                                                       blank=blank['blank'],
                            ...
                            for pattern_density in pattern_density_list:
                                ...
                                # Layer
                                layer_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0,
                                                                                 tool=tool['tool'],
                                                                                 node=node['node'],
                                                                                 tone=tone['tone'],
                                                                                 blank=blank['blank'],
                                ...
                                for layer in layer_list:
                                    ...
                                    layer_no = len(
                                        ...
                                        [blank_no]['children'][grade_no]['children'][pattern_density_no][
                                            'children']) - 1
                                    # level
                                    level_list = unreleased_info_alta.objects.filter(lock=tree_type, is_delete=0,
                                                                                     tool=tool['tool'],
                                                                                     node=node['node'],
                                                                                     tone=tone['tone'],
                                                                                     blank=blank['blank'],
...
@csrf_exempt
def unreleased_alta_export(request):
...
@csrf_exempt
def unreleased_alta_download(request):
    ...
    file_name = 'Mask_DP_infotree_unreleased_alta_template.xlsx'
...
def write_excel(customer, tool, node, tone, blank, grade, pattern_density, layer):
    ...
    '''建立第一個sheet:sheet1'''
    sheet1 = f.add_sheet(u'unreleased_alta', cell_overwrite_ok=True)  # 建立sheet

    tree_data = unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone, blank=blank,
    ...
    excel_head_name = [u'customer', u'tool', u'node', u'tone', u'blank', u'grade', u'pattern_density', u'layer',
                       u'level', u'type', u'item', u'value', u'version']
    row = [excel_head_name]
    for data in tree_data:
        data_list = [data.customer, data.tool, data.node, data.tone, data.blank, data.grade, data.pattern_density,
                     data.layer, data.level, data.type, data.item, data.value, data.version]
        print(data_list)
...
class unreleased_alta_import(BaseView):

    def get(self, request):
        if request.method == 'GET':

            cur_path = os.path.abspath(os.path.dirname(__file__)).split('Mask_DP')[0]
            upl_path = os.path.join(os.path.dirname(cur_path), 'Mask_DP_infotree_import/')
            if not os.path.exists(upl_path):
                os.mkdir(upl_path)

            upl_path = upl_path.replace('\\', '/')

            sub_id = 'INFOTREE_UNRELEASED_ALTA'
            ...
            check_type = 'md5'
            type_no = '0'
            callback_api = "/infotree/unreleased_alta/import_check/"
            # callback_api = ''
...
class unreleased_alta_import_check(BaseView):
    def post(self, request):
        if request.method == 'POST':
            ...
            excel = pd.read_excel(file_path, sheet_name='unreleased_alta')
            ...
            level = str(excel.iloc[0, 9])
            type = str(excel.iloc[0, 10])
            cs_command = str(excel.iloc[0, 11])
            value = str(excel.iloc[0, 12])
            limit_type = str(excel.iloc[0, 13])
            limit = str(excel.iloc[0, 14])

            # 判斷欄位名稱
            if not (group == 'Group' and customer == 'Customer' and tool == 'Tool' and node == 'Node' and \
                    tone == 'Tone' and blank == 'Blank' and \
                    grade == 'Grade' and pattern_density == 'Pattern density' and layer == 'Layer' and \
                    level == 'Level' and type == 'Type' and cs_command == 'Item' and value == 'Value' and \
                    limit_type == 'Limit_type' and limit == 'Limit'):
            ...
            try:
                ...
                while i < excel_len:
                    ...
                    _o.tool = str(excel.iloc[i, 1])
                    _o.node = str(excel.iloc[i, 2])
                    _o.tone = str(excel.iloc[i, 3])
                    _o.blank = str(excel.iloc[i, 4])
                    _o.grade = str(excel.iloc[i, 5])
                    _o.pattern_density = str(excel.iloc[i, 6])
                    _o.layer = str(excel.iloc[i, 7])
                    _o.level = str(excel.iloc[i, 8])
            ...
            param_list = info_tree_data_import_temp.objects.all().values('tool', 'node', 'tone', 'blank', 'grade',
            ...
            for gp in gp_list:
                ...
                temp_count = info_tree_template.objects.filter(is_delete=0, machine_type__contains='MS03').count()
                ...
                gp_dis_list = info_tree_data_import_temp.objects.filter(group=gp['group']).values(
                    'tool', 'node', 'tone', 'blank', 'grade', 'pattern_density', 'layer').distinct()
                gp_dis_count = gp_dis_list.count()
                if not gp_dis_count == 1:
                    msg = "Group : " + str(
                        gp['group']) + ", [tool,noe,tone,blank,grade,pattern_density,layer] 不相同"
                    data = {"success": False, "msg": msg}
                    return JsonResponse(data, safe=False)
                ...
                gp_val_list = info_tree_data_import_temp.objects.filter(group=gp['group']).values('cs_command')
                gp_cs_dis_val_list = info_tree_data_import_temp.objects.filter(group=gp['group']).values(
                    'cs_command')
                i = 0
                while i < gp_count:
                    print(i, gp_val_list[i]['cs_command'], gp_cs_dis_val_list[i]['cs_command'])
                    if gp_val_list[i]['cs_command'] != gp_cs_dis_val_list[i]['cs_command']:
                        msg = "Group : " + str(gp['group']) + " cs_command error"
                        data = {"success": False, "msg": msg}
                        return JsonResponse(data, safe=False)
                    i += 1
                ...
                infotree_data_count = unreleased_info_alta.objects.filter(tool=gp_dis_list[0]['tool'],
                                                                          node=gp_dis_list[0]['node'],
                                                                          tone=gp_dis_list[0]['tone'],
                                                                          blank=gp_dis_list[0]['blank'],
                                                                          grade=gp_dis_list[0]['grade'],
                                                                          pattern_density=gp_dis_list[0][
                                                                              'pattern_density'],
                                                                          layer=gp_dis_list[0]['layer'],
                                                                          is_delete=0).count()
                ...
                check_data_res, check_data_msg = self.check_data(gp['group'], temp_option_dict)
            ...
            # 插入數據到 unreleased_info_alta
            try:
                with transaction.atomic():
                    for _gp in gp_list:
                        group = int(unreleased_info_alta.objects.all().values('group').distinct().order_by('-group')[0][
                                        'group']) + 1
                        ...
                        for temp_gp_data in _temp_gp_data:
                            ...
                            _o.level = temp_gp_data.level
                            _o.type = temp_gp_data.type
                            _o.item = temp_gp_data.cs_command
                        ...
                        unreleased_info_alta.objects.bulk_create(temp_gp_data_list)

            except Exception as e:
                print(e)
                data = {"success": False, "msg": "Excel data insert into unreleased_info_alta failed"}
                return JsonResponse(data, safe=False)
    ...
    def check_data(self, gp, temp_option_dict):
        try:
            with transaction.atomic():
                ...
                print(_data.count())
                for data in _data:
                    ch_res, ch_msg = self.check_data_one(data.level, data.cs_command, data.value, data.limit_type,
                                                         temp_option_dict)
                    print(data.cs_command, ch_res, ch_msg)
                    if not ch_res:
                        return False, ch_msg
            ...
            print(e)
            return False, 'check data failed'

    def check_data_one(self, level, cs_command, value, limit_type, temp_option_dict):
        global s, msg, temp

        temp = info_tree_template.objects.get(level=level, machine_type__contains='MS03', cs_command=cs_command,
                                              is_delete=0)
        ...
        for case in switch(str(cs_command)):
            ...
            if case('PrintMode'):
                ...
                if data in temp_option_dict['PrintMode']:
                    s = True
                else:
                    s = False
                    msg = 'PrintMode error'
            ...
            if case('ManualAlignAction'):
                data = str(value)

                if data in temp_option_dict['ManualAlignAction']:
                    s = True
                else:
                    s = False
                    msg = 'ManualAlignAction error'
                break
...
@csrf_exempt
def unreleased_alta_release(request):
    customer = request.POST.get('customer')
    tool = request.POST.get('tool')
    node = request.POST.get('node')
    tone = request.POST.get('tone')
    ...
    unre_info_alta_count = unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone,
                                                               blank=blank,
                                                               grade=grade, pattern_density=pattern_density,
                                                               layer=layer, is_delete=0).exclude(value='').count()

    temp_count = info_tree_template.objects.filter(machine_type__contains='MS03').exclude(is_delete=1).count()

    print(unre_info_alta_count, temp_count)

    if unre_info_alta_count != temp_count:
        data = {"success": False, "msg": "Value can't be null"}
        return JsonResponse(data, safe=False)

    unre_info_id = \
        unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone, blank=blank,
                                            grade=grade,
                                            pattern_density=pattern_density, layer=layer, is_delete=0,
                                            ).values('id', 'value')[0]

    print(unre_info_id)
    if unre_info_id['value'] == 'Y':
        unreleased_mdt_count = unreleased_mdt.objects.filter(unreleased_id=unre_info_id['id'],
                                                             is_delete=0).count()
        if unreleased_mdt_count == 0:
            data = {"success": False, "msg": "MDT value is 'Y'.  But not found data"}
            return JsonResponse(data, safe=False)

    _unre = unreleased_info_alta.objects.filter(customer=customer, tool=tool, node=node, tone=tone, blank=blank,
                                                grade=grade,
                                                pattern_density=pattern_density, layer=layer, is_delete=0)
    group = _unre[0].group
...
class unreleased_alta_mdt_list(BaseView):

    def get(self, request):
        if request.method == 'GET':
            id = request.GET.get('id')
            lock = request.GET.get('lock')
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_mdt_list.html',
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            info_tree_datalist = unreleased_mdt.objects.filter(q).order_by('id').values()[
                                 self.startIndex: self.endIndex]
...
class unreleased_alta_mdt_view(BaseView):

    def get(self, request):
        if request.method == 'GET':
            id = request.GET.get('id')
            _o = unreleased_mdt.objects.get(id=id)
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_mdt_form.html', {"mdt": _o})


class unreleased_alta_mdt_add(BaseView):

    def get(self, request):
        if request.method == 'GET':
            _o = unreleased_mdt()
            id = request.GET.get('unreleased_id')
            return render(request, 'info_tree/unreleased_alta/infotree_unreleased_alta_mdt_form.html',
...
class unreleased_alta_mdt_edit(BaseView):
...
class unreleased_alta_mdt_del(BaseView):
    def get(self, request):
        if request.method == 'GET':

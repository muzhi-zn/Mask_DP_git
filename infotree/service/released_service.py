...
from infotree.models import released_info, released_mdt
...
class released_service(BaseService):
    ...
    def del_released(self, released_data):
        ...
        with transaction.atomic():
            mdt_id = released_info.objects.filter(tool=tool,
                                                  node=node,
                                                  tone=tone,
                                                  blank=blank,
                                                  grade=grade,
                                                  ...
                                                  layer=layer,
                                                  is_delete=0, cs_command='MDT').values('id')[0]['id']

            _o = released_info.objects.filter(tool=tool,
                                              node=node,
                                              tone=tone,
                                              blank=blank,
                                              grade=grade,
            ...
            released_mdt.objects.filter(released_id=mdt_id, is_delete=0).update(is_delete=1)

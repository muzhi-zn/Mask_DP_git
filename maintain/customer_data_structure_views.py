...
from maintain.models import ftp_data_table
from datetime import datetime
from maintain.service.customer_data_structure_service import CustomerDataService
class customer_data_structure_list(BaseView):

    def get(self, request):
        if request.method == 'GET':
            return render(request, 'maintain/customer_data_structure.html')
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if customer and product_name and date:
                ...
                info_tree_datalist = ftp_data_table.objects.filter(q).order_by('id').values()[
                                     self.startIndex: self.endIndex]

                data['total'] = ftp_data_table.objects.filter(q).count()
...
class customer_data_structure_pass(BaseView):

    def get(self, request):
        data = {}
        ids = request.GET.get('ids')
        c_f = CustomerDataService()
        c_f.id = ids
        return CustomerDataService().pass_data_in(c_f, request)

class customer_data_structure_pass_cancel(BaseView):
    ...
    def get(self, request):
        ...
        c_f = CustomerDataService()
        c_f.id = ids
        return CustomerDataService().cancel_pass_data_in(c_f, request)


def customer_data_structure_tree_json(request):
    q = Q()
    q.connector = 'and'
    q.children.append(('is_delete', 0))
...
#    tree_type = int(request.GET.get('tree_type'))
    ...
    # Tool
    customer_list = ftp_data_table.objects.filter(q).order_by('customer').values(
        'customer').distinct()
    print(customer_list)
    ...
    for customer in customer_list:
        ...
        product_name_list = ftp_data_table.objects.filter(q).values(
        ...
        for product_name in product_name_list:
            ...
            date_list = ftp_data_table.objects.filter(q).values('date').distinct()
...
@csrf_exempt
def customer_data_structure_export(request):
...
def get_customer(request):
    customer = ftp_data_table.objects.filter(is_delete='0')
    customer_list = list()

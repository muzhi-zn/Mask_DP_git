...
from maintain.service.data_in_service import DataInService
class data_in_list(BaseView):

    def get(self, request):
        if request.method == 'GET':
            return render(request, 'maintain/data_in_management.html')
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if customer and product_name and date:
                ...
                info_tree_datalist = data_in_table.objects.filter(q).order_by('id').values()[
                                     self.startIndex: self.endIndex]

                data['total'] = data_in_table.objects.filter(q).count()
...
class data_in_pass(BaseView):
    ...
    def get(self, request):
        ...
        c_f = DataInService()
        c_f.id = ids
        return DataInService().pass_data_in(c_f, request)

class data_in_pass_cancel(BaseView):
    ...
    def get(self, request):
        ...
        c_f = DataInService()
        c_f.id = ids
        return DataInService().cancel_pass_data_in(c_f, request)


def data_in_tree_json(request):
    q = Q()
    q.connector = 'and'
    q.children.append(('is_delete', 0))
...
#    tree_type = int(request.GET.get('tree_type'))
    ...
    # Tool
    customer_list = data_in_table.objects.filter(q).order_by('customer').values(
        'customer').distinct()
    print(customer_list)
    ...
    for customer in customer_list:
        ...
        # Node
        product_name_list = data_in_table.objects.filter(q).values(
        ...
        for product_name in product_name_list:
            ...
            date_list = data_in_table.objects.filter(q).values('date').distinct()
            print(date_list)
...
def get_customer(request):
    customer = data_in_table.objects.filter(is_delete='0')
    customer_list = list()

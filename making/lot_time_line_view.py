...
class LotTimeline(BaseView):
    """lot时间线"""
    def get(self, request):
        return render(request, 'lot_timeline.html')
...
class LotTrailDel(BaseView):
    def post(self,request):
        lot_trail_id = request.POST.get('lot_trail_id')
        _o = lot_trail()
        _o.id = lot_trail_id
        return LotTrailService().del_lot_trail(_o)
...
def get_lot_list(request):
    ...
    lot_list = lot_info.objects.filter(tip_no=tip_no, lot_owner_id=user_id, is_delete=0)
    _list = []
    for lot in lot_list:
        item = {}
        item['name'] = lot.lot_id
        item['id'] = lot.id
        item['text'] = lot.lot_id
        _list.append(item)
    if _list.__len__()==0:
        item = {}
        item['name'] = 'No data'
